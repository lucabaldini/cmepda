...
\begin{frame}
  ...
  \framesubtitle{Digression - namespaces}
  ...
  \begin{itemize}
    ...
    \item A \alert{namespace} in Python is a essentially a ditcionary of \emph{unique} names, each one
          referncing an object (which can be anything: a variable, a function, a class etc\dots)
    \medskip
    \item The '.' (dot) operator is used to access objects in that namespace (\emph{scope resolution})
    \medskip
...
\begin{frame}
  \frametitle{Class members}
  \framesubtitle{class attributes vs instance attributes}
  
  \begin{itemize}
    \small
    \item Differently from other languages, that require you to declare the member
          variables explicitly in the class body (e.g. C++), in Python you can
          just do an assignment in that namespace, everywhere in the code
    \medskip
    \item You can even add class members outside the class code, though that is
...
\begin{frame}
  \frametitle{Class members}
  \framesubtitle{Instance attributes}
  \input{pygments/class_members}
\end{frame}
...
\begin{frame}
  \frametitle{Class members}
  \framesubtitle{Class attributes and their strange behaviour}
