...
\begin{frame}
  ...
  \begin{itemize}
    \item Working with containers like lists or dictionaries, you may have noticed
          that they can do many things besides holding the data
    \medskip
    \begin{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \medskip
    \item As the name suggests, OOP is often adopted for modeling systems that are naturally
          described in terms of objects
    \begin{itemize}
      \smallskip
      \item Physical simulations: Geant4 (C++), SimPy (Python)
      \smallskip
      \item Graphics engines and game development: Unity (C\#), Unreal Engine (C++)
      \smallskip
      \item Graphical User Interfaces (GUI): Qt / PySide (C++/Python), JavaFX (Java), .NET WinForms (C\#)
    \end{itemize}
    \medskip
    \item But also used in Enterprise applications, Web frameworks, Compilers, OS, Robotics, Embedded Systems and so on.
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \smallskip
    \item That does not necessarily mean it is the best one -- nor the right tool for any job \\
    \tiny
    \url{https://en.wikipedia.org/wiki/Object-oriented_programming\#Criticism}
    \normalsize
    \smallskip

    \item There is a number of famous programmers who really dislike it (e.g. \emph{Linus Torvalds})
    \medskip
...
\begin{frame}
  ...
  \centering\includegraphics[width=0.7\textwidth]{tiobe2025.png}
...
\begin{frame}
  ...
  \item You can imagine a class like a project, which is used to
        describe how objects are built and how they work
...
\begin{frame}
  ...
  \item Let's consider a familiar object, like a television. It has:
    ...
    \item A behaviour
      ...
      \item And don't forget you need to plug it in before use!
      \end{itemize}
    \end{itemize}
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \smallskip
      ...
      \begin{itemize}
        ...
        \item You will probably have the next\_channel() and previous\_channel() functions for zapping and so on\dots
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item In Python the constructor is the \emph{\_\_init\_\_} method%
          \footnote{Actually the real constructor -- the function responsible for
    ...
    \medskip
    \item Class methods like \emph{\_\_init\_\_}, with the name surrounded by two underscores,
          are called \alert{special} methods or \alert{dunder} methods.
    \medskip
    \item Good practice: \textbf{define all your class attributes inside the constructor!}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \medskip
    \item A \alert{namespace} in Python is essentially a dictionary of \emph{unique} names, each one associated to an object (which can be anything: a variable, a function, a class etc\dots)
    \medskip
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item You can access objects in the local namespace (and those above --
          ...
          the '.' (dot) operator.
    \medskip
    \item The space of visibility of a variable is called its \alert{scope}.
  \end{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
    \small
    \item Each class has a namespace. Plus, each instance of the class gets its own additional namespace
    \medskip
...
\begin{frame}{Short summary (OOP basics)}
\begin{itemize}
  \item \textbf{Object-Oriented Programming (OOP)} is a widely used paradigm, 
  	supported by most modern languages (including Python).  
  \smallskip
  \item An \textbf{object} has:
    ...
    \end{itemize}
  \smallskip
  \item A \textbf{class} is a blueprint for creating objects; each object is an
  	\textbf{instance} of a class. 
  \smallskip
  \item In Python:
    \begin{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item That way it is easier to find bugs: you know that, if something is wrong
          with an object, the problem lies inside the class code
    \bigskip
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item "I have this super-cool algorithm to solve the problem I am working on:
             I will code it carefully, then put quickly together some basic
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Changing the interface may break every other piece of code that uses it.
          You want to do that \emph{as little as possible}
    \medskip
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \medskip
    \item You may have a class Detector and a class for each particle that you need to simulate
    \medskip
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \medskip
    \item Inheritance is transitive: if class C inherits from class B, and class B inherits from class A, then class C is also a child of class A (and possess all its functionalities)
  \end{itemize}
